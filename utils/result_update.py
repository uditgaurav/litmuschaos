...
parser.add_argument("--pipeline_result",
       help=" github pipeline result: PASSED or FAILED")
...
def fetch_file_content():
    ...
    if file_content.find('|')>0:
        new_pipeline = '|     {}           |  {}           | {}  | {} |'.format(pipeline_url,time_stamp,tag,pipeline_result)
        index = content_list.index('| Pipeline ID |   Execution Time        | Release Version |Pipeline Result   |')
        content_list.insert(index+2,new_pipeline)
        updated_file_content = ('\n').join(content_list)

    # creating result's table for first pipeline result entry 
    else:
        updated_file_content =  '| Pipeline ID |   Execution Time        | Release Version |Pipeline Result   |\n'
        updated_file_content = updated_file_content + (' |---------|---------------------------| --------------|--------|\n')
        updated_file_content = updated_file_content + (' |    {}   |  {}           |  {}     |{}  |\n'.format(pipeline_url,time_stamp,tag, pipeline_result))
