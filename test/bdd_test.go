...
var _ = BeforeSuite(func() {
	...
	err = exec.Command("kubectl", "create", "-f", "https://raw.githubusercontent.com/litmuschaos/chaos-operator/master/deploy/chaos_crds.yaml").Run()
	if err != nil {
		fmt.Println(err)
	}

	//Creating rbacs
	err = exec.Command("kubectl", "create", "-f", "https://raw.githubusercontent.com/litmuschaos/chaos-operator/master/deploy/rbac.yaml").Run()
	if err != nil {
		fmt.Println(err)
	}
...
var _ = Describe("BDD on chaos-operator", func() {
	...
	Context("Check for the custom resources", func() {
		...
		It("Should check for pod-delete", func() {
			...
			result, err := clientSet.ChaosResults("litmus").Get("engine-nginx-pod-delete",metav1.GetOptions{})
			Expect(err).To(BeNil())
			Expect(string(result.ExperimentStatus.Verdict)).To((Equal("pass")))
	})
})
